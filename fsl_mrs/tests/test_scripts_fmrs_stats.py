...
def test_fmrs_stats_list_of_dir(tmp_path):
    ...
    subprocess.run([
        'fmrs_stats',
        '--data', str(tmp_path / 'results_list'),
        '--output', str(tmp_path / 'out_file'),
        '--overwrite'])

    df_file = pd.read_csv(tmp_path / 'out_file' / 'group_stats.csv', index_col=0, header=[0])

    assert df_file.equals(df)
